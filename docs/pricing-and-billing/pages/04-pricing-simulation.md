...
- 所有 UI 文案，包括模拟场景、客户信息、表单字段、结果指标、定价模型、状态、按钮、校验和提示，必须使用 i18n key，不得硬编码中文或英文。
- 新增文案统一维护在 `src/locales/zh-CN/pages.ts`、`src/locales/en-US/pages.ts` 和对应的 `menu.ts` 中，使用 `pages.pricing.simulation.*` 与菜单专属 key。
- 客户名称、产品代码、币种代码和 Mock 数值可以保留业务标准值；结果标签和枚举显示名称必须国际化。
- 切换 `zh-CN` / `en-US` 后，模拟表单、计算结果、对比卡片、详情和导出提示都必须显示对应语言。
